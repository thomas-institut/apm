...
import {WitnessUpdateInfo} from "@/Api/DataSchema/WitnessUpdates";

interface WitnessUpdateDialogOptions {
  ctData: CtDataInterface,
  witnessIndex: number,
  newWitnessInfo: WitnessUpdateInfo,
  getWitnessData: (witnessId: string) => Promise<any>,
  witnessDiffCalculator: WitnessDiffCalculator,
  updateWitness: (witnessIndex: number, changes: any, newWitness: any) => Promise<boolean>,
  icons: {
    busy: string, checkOK: string, checkCross: string
  },
  debug?: boolean
}

export class WitnessUpdateDialog {
  private options: WitnessUpdateDialogOptions;
  private readonly ctData: CtDataInterface;
  private icons: { busy: string; checkOK: string; checkCross: string };
  private debug: boolean;

  constructor(options: WitnessUpdateDialogOptions) {

    this.options = options;
    this.ctData = options.ctData;
  ...
  go() {
    ...
    let dialogHtml = this._getDialogHtml(witnessIndex, this.ctData['witnessTitles'][witnessIndex], ApmFormats.timeString(currentWitness.timeStamp ?? ''), ApmFormats.time(newWitnessInfo['lastUpdate']));
    ...
    $(modalSelector).on('shown.bs.modal', () => {
      ...
      .then(changeData => {
        ...
        let changesToShow = changes.ctChanges.filter((ch) => {
          return !(ch.type === 'replace' && ch.currentToken.text === ch.newToken.text && ch.currentToken.normalizationSource !== undefined && ch.currentToken.normalizationSource === 'automaticCollation');
        });
        let html = '';
        html += '<h5>Changes:</h5>';
        html += '<ul>';
        if (changesToShow.length !== 0) {
          ...
          function getTokenText(token: WitnessToken) {
            if (token === undefined) {
          ...
          for (const ctChange of changesToShow) {
            ...
            switch (ctChange.type) {
              case 'insertColAfter':
                if (ctChange.afterCol === undefined) {
                  console.error('After column is undefined');
                  return;
                }
  ...
  _getDialogHtml(witnessIndex: number, witnessTitle: string, currentVersion: string, newVersion: string) {
