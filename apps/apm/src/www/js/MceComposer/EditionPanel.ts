...
interface EditionPanelOptions extends PanelOptions {
  mceData: MceDataInterface,
  showLoadingDataMessage?: boolean,
  icons?: { [key: string]: string },
  getUpdateStatuses: () => Promise<boolean[]>,
  updateChunk: (chunkIndex: number) => Promise<void>,
  deleteChunk: (chunkIndex: number) => Promise<void>,
  updateChunkOrder: (newOrder: number[]) => Promise<void>,
  updateAutoMarginalFoliation: (newAutoMarginalFoliation: number[]) => Promise<void>,
  updateSigla: (newSigla: string[]) => Promise<void>,
  updateSiglaGroups: (newSiglaGroups: SiglaGroupInterface[]) => Promise<void>,
  updateChunkBreak: (chunkIndex: number, newBreak: string) => Promise<void>,
}
...
export class EditionPanel extends Panel {
  ...
  private updateStatuses!: boolean[];
  private loading: boolean;
  private options: EditionPanelOptions;

  constructor(options: EditionPanelOptions) {
    super(options);

    let oc = new OptionsChecker({
      context: 'EditionPanel', optionsDefinition: {
        mceData: {type: 'object', required: true},
        showLoadingDataMessage: {type: 'boolean', default: true},
        icons: {type: 'object', default: defaultIcons},
        getUpdateStatuses: {
          type: 'function', default: (force: boolean) => {
            if (force) {
              console.log(`Faking force get update statuses`);
            }
            return Promise.resolve(this.mceData.chunks.map(() => {
              return 'false';
            }));
          }
        },
        updateChunk: {
          type: 'function', default: (chunkIndex: number): Promise<void> => {
        ...
        updateChunkOrder: {
          type: 'function', default: (newOrder: number[]) => {
        ...
        updateAutoMarginalFoliation: {
          type: 'function', default: async (newAutoMarginalFoliation: number[]) => {
            console.log(`New auto marginal foliation`);
            console.log(newAutoMarginalFoliation);
          }
        },
        updateSigla: {
          type: 'function', default: (newSigla: string[]) => {
        ...
        updateSiglaGroups: {
          type: 'function', default: (newSiglaGroups: SiglaGroupInterface[]) => {
        ...
        updateChunkBreak: {
          type: 'function', default: (chunkIndex: number, newBreak: string) => {
    ...
    this.loading = this.options.showLoadingDataMessage ?? true;
  ...
  _genOnClickDeleteSiglaGroup(i: number) {
    return (ev: any) => {
      ...
      SiglaGroupsUI.confirmDeleteSiglaGroup(this.mceData.siglaGroups, i, this.mceData.sigla).then(() => {
        ...
        this.mceData.siglaGroups = this.mceData.siglaGroups.filter((_sg, index) => {
          return index !== i;
        });
        this.options.updateSiglaGroups(this.mceData.siglaGroups).then();
  ...
  __genSiglaTable() {
    return ['<table class="edition-panel-table sigla-table">', '<tr><th>Witness</th><th>Siglum</th><th>Marg. Fol.</th><th></th></tr>', this.mceData.witnesses.map((witness, witnessIndex) => {
      return `<tr>
                <td>${witness.title}</td>
  ...
  __genChunksTable() {
    ...
    if (this.mceData.chunkOrder === undefined) {
      this.mceData.chunkOrder = [];
    }
    this.verbose && console.log(`Generating chunk table with ${this.mceData.chunks.length} chunks and ${this.mceData.chunkOrder.length} ordered chunks`);
  ...
  _getBreakLabel(breakType: string) {
    switch (breakType) {
      case 'paragraph':
        return 'Paragraph';
   ...
   */
  updateData(mceData: MceDataInterface) {
  ...
  _setupEventHandlers() {
    ...
    const chunkOrder = this.mceData.chunkOrder;
    // Chunk table
    this.mceData.chunks.forEach((chunk, chunkIndex) => {
      ...
      let chunkPosition = chunkOrder.indexOf(chunkIndex);
      ...
      let toggle = new MultiToggle({
        ...
        buttonDef: [{label: 'None', name: 'none', helpText: 'No break with next chunk'}, {
          label: 'Paragraph', name: 'paragraph', helpText: "Next chunk starts in new paragraph"
        },],
      });
      toggle.on('toggle', (ev: any) => {
  ...
  _genOnConfirmSiglumEdit(siglumIndex: number) {
    return (ev: any) => {
      ...
      if (this.mceData.sigla.indexOf(newText) !== -1) {
        ...
        transientAlert(infoElement, '', "Given siglum '" + newText + "' already exists, no changes made", 2000, 'slow');
        editor.setText(this.mceData.sigla[siglumIndex]);
        return false;
      }
      // Change the siglum
  ...
  _genOnClickDeleteChunk(chunkIndex: number) {
    return () => {
      let confirmDialog = new ConfirmDialog({
        ...
        acceptFunction: () => {
          ...
          button.html(`Deleting... ${this.icons.busy}`);
          chunkInfoSpan.html('');
  ...
  _genOnClickUpdateChunk(chunkIndex: number) {
    return () => {
      let button = $(`${this.containerSelector} button.update-chunk-${chunkIndex}`);
      let chunkInfoSpan = $(`${this.containerSelector} .chunk-info-span-${chunkIndex}`);
      let currentButtonHtml = button.html();
      button.html(`Updating... ${this.icons.busy}`);
      chunkInfoSpan.html('');
  ...
  _setupSiglaGroupsTable() {
    ...
    this.mceData.siglaGroups.forEach((_sg, i) => {
  ...
  _genOnClickUpDownButton(chunkIndex: number, position: number, direction: string) {
    return (ev: any) => {
  ...
  private __genLoadingStateHtml(loadingMessage: string) {
    return `<div class='empty-chunks-info'>
