...
export class DocPage extends NormalPage {
  ...
  constructor(options: DocPageOptions) {
    ...
    const defaults = {
      canDefinePages: false, canEditDocuments: false, params: [], selectedPage: 0
    };
    ...
    Object.keys(this.pages).forEach((key) => {
      ...
      if (this.pages[pageId].foliation === null) {
        this.pages[pageId].foliation = this.pages[pageId].sequence.toString();
      }
    });
  ...
  async initPage() {
    ...
    new MetadataEditor({
      ...
      infoStringProviders: [{
        name: 'docShortInfo', provider: async () => {
          return `${this.doc.numTranscribedPages} of ${Object.keys(this.doc.pages).length} pages transcribed`;
        }
      }]
    });
    ...
    this.thumbnailToggle = new MultiToggle({
      ...
      buttonDef: [{label: 'None', name: 'none'}, {label: 'Tiny', name: 'tiny'}, {
        label: 'Small',
        name: 'small'
      }, {label: 'Medium', name: 'medium'}, {label: 'Large', name: 'large'}, // { label: 'X-Large', name: 'x-large'},
      ],
    });
    ...
    $('button.first-btn').on('click', () => {
      this.selectPage(this.firstPage, true);
    });
    $('button.last-btn').on('click', () => {
      this.selectPage(this.lastPage, true);
    });
    ...
    this.pageInfoPopper = Popper.createPopper(// @ts-ignore this should be defined
      pageNumberSpan.get(0), this.pageListPopoverDiv.get(0), {
    ...
    this.split = Split({
      columnGutters: [{
        track: 1, element: dividerElement,
    ...
    new WidgetAddPages('div.add-pages-widget-container', this.docId, Object.keys(this.doc.pages).length);
    new SetLanguage({
  ...
  setDefaultLanguageForAllPages(newLanguage: number) {
    let pageDefs = [];
    for (let i = 0; i < Object.keys(this.doc.pages).length; i++) {
      pageDefs.push({
        docId: this.doc.docInfo.id, page: i + 1, lang: newLanguage
      });
    }
    return new Promise((resolve, reject) => {
      $.post(urlGen.apiBulkPageSettings(), {data: JSON.stringify(pageDefs)}).done(() => {
        resolve('');
      }).fail((resp) => {
  ...
  getPageSequenceToPageKeyMap(pages: Record<number, OverkillPageInfoItem>) {
    ...
    Object.keys(pages).forEach((pageKey) => {
      ...
      map[pages[pageId].sequence] = pageId;
    });
    return map;
  }
...
`;
  ...
  getPageAdminHtml() {
    ...
    let definePagesHtml = this.canDefinePages ? `<a class="btn btn-sm btn-primary" href="${defineDocPagesUrl}">Define pages</a>` : '';
  ...
  async genWorkInfoHtml() {
    ...
    for (const workDareId in this.chunkInfo) {
      ...
      const workData = await this.apiClient.getWorkData(workDareId);
      const authorName = await this.apiClient.getEntityName(workData.authorId);
      html += `<li><a href="${urlGen.sitePerson(Tid.toBase36String(workData.authorId))}">${authorName}</a>, 
            <a href="${urlGen.siteWorkPage(Tid.toBase36String(workData.entityId))}"><em>${workData.title}</em> (${workDareId})</a>`;
      html += '<ul><li>';
      let tdArray = [];
  ...
  async getChunkLabelHtml(work: string, chunk: number) {
    ...
    if (!this.isChunkValid(work, chunk)) {
      ...
      const authorName = this.versionInfo[work][chunk].authorTid === 0 ? '' : await this.apiClient.getEntityName(this.versionInfo[work][chunk].authorTid);
  ...
  getChunkLink(work: string, chunk: number) {
    ...
    // @ts-ignore
    return '<a href="' + urlGen.siteChunkPage(work, chunk) + '" target="_blank" title="Open chunk page ' + work + '-' + chunk + ' in new tab">' + icon + '</a>';
  }
  ...
  async getAuthorLink(authorId: number) {
    ...
    let url = urlGen.sitePerson(Tid.toBase36String(authorId));
    const authorName = await this.apiClient.getEntityName(authorId);
    return `<a href="${url}" title="View user profile" target="_blank">${authorName}</a>`;
  }
  ...
  async getLastSavesHtml() {
    ...
    for (const i in this.lastSaves) {
      ...
      html += '<li> Page ' + this.getPageLinkFromPageId(versionInfo.pageId, versionInfo.column) + ', ' + formattedTime + ' by ' + authorLink + '</li>';
    }
    html += '</ol>';
    return html;
  }
