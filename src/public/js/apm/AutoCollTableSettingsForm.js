...
class AutomaticCollationTableSettingsForm {
  ...
  constructor(containerSelector) {
    // Event generated by the form
    this.cancelEventName = 'cancel'
    this.applyEventName = 'apply'
    this.optionsChangeEventName = 'options-change'
  ...
  getTitleFromOptions(langDef, availableWitnesses = false, options = false) {
    ...
    let title = ''
    let numWitnesses = options.witnesses.length
    if (numWitnesses === 0) {
      numWitnesses = availableWitnesses.length
    }
    
    title += langDef[options.lang].name + ', '
  ...
  genOnClickIgnorePunctuationCheckbox() {
    let thisObject = this
    return function() {
      thisObject.dispatchEvent(thisObject.optionsChangeEventName, thisObject.getOptions())
    }
  }
