...
class ApiTranscription extends ApiController
{
    ...
    public function getList(Request $request, Response $response): Response
    {
        ...
        foreach($docs as $docId => $pageNumbers) {
            try {
                ...
                // should never happen
                $this->logger->error("Document not found getting info for doc $docId: " . $e->getMessage());
            ...
            if ($docInfo['image_source'] === Entity::ImageSourceBilderberg) {
    ...
    public function getTranscription(Request $request, Response $response): Response
    {
        ...
        // Disabled 2024-12-18 when migrating to new DocumentManager
        $this->logger->warning("API call to disabled function: getTranscription getting info for doc $docId page $pageNumber");
        return $this->responseWithJson($response, []);
//
//
//        $docId = intval($request->getAttribute('docId'));
