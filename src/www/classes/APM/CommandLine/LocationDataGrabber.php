 ...
 *
 * Commandline utility to grab country, city and institution data for all location codes in dare and apm, with the help of wikidata; writes the data into csv-files.
...
class LocationDataGrabber extends CommandLineUtility
{
    ...
    public function main($argc, $argv): bool
    {
        ...
        switch ($operation) {
            ...
            case 'collectEssentials':
                $this->collectEssentials();
                break;

            case 'showCityNamesFromWikidata':
                $city = $this->getCityNamesByUnlocodeFromWikidata($argv[2]);
                print_r($city);
                break;

            case 'showCountryNamesFromWikidata':
...
Available operations are:
  collectAll - collects all accessible location data from dare, apm and wikidata and writes them into multiple csv-files.
  collectEssentials - collects only specific location data (location code, country, city, institution) from dare. apm and wikidata and writes them into a single csv-file.
  
  showCityNamesFromWikidata [arg] - returns the city names to a given UNLOCODE in english, german and italian, retrieved from wikidata.
  showCountryNamesFromWikidata [arg] - returns the country names to a given ISO3166-1-alpha-2-code in english, german and italian, retrieved from wikidata.
...
END;
    ...
    /**
     * grabs all data from dare and wikidata and writes them into separate files as well as into a file which contains all data
     * @return void
     * @throws DocumentNotFoundException
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    private function collectAll(): void
    {
     ...
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    private function collectEssentials(): void
    {
        print("getting institutions data from exported dare table...\n");
        $institutionDataFromDare = $this->getInstitutionsDataFromDare();
