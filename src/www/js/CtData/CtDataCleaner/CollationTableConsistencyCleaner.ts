...
export class CollationTableConsistencyCleaner extends CtDataCleaner {
  ...
  checkAndFixCollationTableConsistency(ctData: CtDataInterface) {
    ...
    for (let wIndex = 0; wIndex < ctData.witnesses.length; wIndex++) {
      ...
      if (columnsInWrongOrder) {
        ...
        let orderedTokenRefs = row.filter( (ref: number) => {return ref!==-1}).sort( (a:number,b:number) => {
          if (a>b) {
            return 1
          }
          if (a < b) {
  ...
  findNextNullRefInArray(theArray: number[], startingIndex: number) {
