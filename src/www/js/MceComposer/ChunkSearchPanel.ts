...
const defaultIcons = {
  alert: '<i class="fas fa-exclamation-triangle"></i>', busy: '<i class="fas fa-circle-notch fa-spin"></i>',
};
...
export class ChunkSearchPanel extends Panel {
  ...
  setupActiveEditionTableButtons(data: any[]) {
    ...
    new DataTable("table.active-editions", {
      paging: true, searching: true, lengthMenu: [25, 50, 100], language: {
        search: 'Filter:'
      }, columns: [// @ts-ignore
        null, // @ts-ignore
        null, // @ts-ignore
        null, // @ts-ignore
        null, {searchable: false, orderable: false},]
    });
    //
  ...
  async genActiveEditionTable(infoArray: any) {
    ...
    let tableIdsInMceData = this.mceData.chunks.map((chunk) => {
