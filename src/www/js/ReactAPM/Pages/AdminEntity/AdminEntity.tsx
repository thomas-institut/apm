...
export default function AdminEntity() {
  ...
  const handleAcceptCancel = async () => {
    ...
    const commands = [{
      command: 'cancel',
      statementId: cancelDialogProps.statement.id,
  ...
  const renderObject = (object: any, valueType: string) => {
    ...
    switch (valueType) {
      ...
      case 'url':
        return <a href={object} className="url-value" target="_blank" rel="noreferrer"><i className="bi bi-link-45deg"></i> {object}</a>;
      default:
  ...
  const renderStatementsTable = (statements: StatementDataInterface[]) => {
    ...
    return (
      <Table striped bordered hover responsive className="statements mt-2">
        <thead>
          <tr>
            <th>Statement Id</th>
            <th>Subject</th>
        ...
        <tbody>
          {sortedStatements.map(statement => {
            ...
            const qualificationItems: React.ReactNode[] = [];
            const cancellationItems: React.ReactNode[] = [];

            statement.statementMetadata.forEach(([predicate, obj], idx) => {
              const item = (
                <div key={`${statement.id}-${predicate}-${idx}`}>
            ...
            statement.cancellationMetadata.forEach(([predicate, obj], idx) => {
              cancellationItems.push(
                <div key={`cancel-${statement.id}-${predicate}-${idx}`}>
                  <EntityLink id={predicate} type="admin" />: {renderObject(obj, getObjectValueType(predicate))}
                </div>
              );
            });
            ...
            return (
              <tr key={statement.id} className={isCancelled ? 'table-secondary text-muted' : ''}>
                <td className={isCancelled ? 'text-decoration-line-through' : ''}>{statement.id}</td>
                <td className={isCancelled ? 'text-decoration-line-through' : ''}><EntityLink id={statement.subject} type="admin" /></td>
                <td className={isCancelled ? 'text-decoration-line-through' : ''}><EntityLink id={statement.predicate} type="admin" /></td>
                <td className={isCancelled ? 'text-decoration-line-through' : ''}>{renderObject(statement.object, getObjectValueType(statement.predicate))}</td>
                    ...
                    <div className="mt-2">
                      ...
                      {predicateDef?.flags?.includes(5) ? (
                        ...
                        <div className="mt-1 d-flex gap-1">
                          {predicateDef?.canBeCancelled && (
                            <Button variant="outline-danger" size="sm" onClick={() => handleCancelStatement(statement)}>Cancel</Button>
                          )}
                          <Button variant="outline-primary" size="sm" onClick={() => handleEditStatement(statement)}>Edit</Button>
                        </div>
                      )}
                    </div>
                  )}
  ...
  const renderAvailablePredicates = (asSubject: boolean) => {
    ...
    return (
      <div className="available-predicates mt-3">
    ...
    <NormalPageContainer>
      ...
      <div className="basic-data border p-3 rounded bg-light mb-4">
        {Object.entries(data).map(([key, val]) => {
          ...
          let valContent;
          if (val === null) valContent = 'null';
          else if (typeof val === 'number') valContent = <EntityLink id={val} type="admin" />;
          else valContent = `'${val}'`;
            ...
            <div>
              ...
              <p className="border p-2 bg-light rounded">
                ...
                <b>Object</b>: {typeof cancelDialogProps.statement.object === 'number' ?
                  <EntityLink id={cancelDialogProps.statement.object} type="admin" /> :
                  `'${cancelDialogProps.statement.object}'`}
              </p>
