...
export default function GenericStatementEditor(props: GenericStatementEditorProps) {
  ...
  const [info, setInfo] = useState<string | React.ReactNode>('');
  ...
  const [qualificationEntities, setQualificationEntities] = useState<Record<number, {id: number, name: string}[]>>({});
        ...
        for (const qp of allowed) {
          ...
          if (def && def.type === Entity.tRelation && def.allowedObjectTypes) {
            ...
            const entityData = await Promise.all(entities.map(async id => ({
                id,
                name: await apiClient.getEntityName(id)
            })));
      ...
      <Modal.Body>
        ...
        <Form>
            ...
            {(predicateDef.allowedQualifications ?? []).map(qp => {
                ...
                return (
                    <Form.Group className="row mb-2" key={qp}>
                        <Form.Label className="col-sm-3">{def.name}</Form.Label>
                        <div className="col-sm-9">
                            {def.type === Entity.tRelation ? (
                                <Form.Select
                                    value={value}
        ...
        {!canSave && changed && validationErrors.length > 0 && (
