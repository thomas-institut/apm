...
export default function Chunk() {
  ...
  const getGeneralData = async (): Promise<GeneralData | null> => {
    ...
    return {
      docTypes: docTypes, languages: languages, workData: workData
    };
  };
  ...
  const getWitnessesForChunk = async () => {
    ...
    return context.apiClient.getWitnessesForChunk(workId, parseInt(chunkId));
  };
  ...
  function witnessInfoText(data: WitnessInfo[], languages: EntityNameTuple[]) {
    ...
    byLanguageCount.forEach(l => {if (l.count > 0) labels.push(`${l.count} ${l.language}`)});
  ...
  const breadCrumb = <Breadcrumb>
    ...
    <Breadcrumb.Item active>Chunk {chunkId}</Breadcrumb.Item>
  </Breadcrumb>;

  const header = <h1><EntityLink id={generalData.workData.authorId}/>, <em>{generalData.workData.title}</em>,
    chunk {chunkId}</h1>;
  ...
  return (<NormalPageContainer>
      ...
      <div className={'chunkPageSection'}>
        ...
        <div><em>... chunk editions will be here...</em></div>
      </div>
      <div className={'chunkPageSection'}>
        <h2>Saved Collation Tables</h2>
        <div><em>... save collation tables will be here...</em></div>
      </div>
      <div className={'chunkPageSection'}>
