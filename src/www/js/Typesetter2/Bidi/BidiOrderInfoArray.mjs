...
export class BidiOrderInfoArray {
   ...
   * @return {boolean}
   */
  static isSingleDirection(boiArray) {
  ...
  static getLevelInfoFromBidiOrderInfoArray(itemDataArray) {
    ...
    let levelInfo = new LevelInfo(currentLevel, 0, -1, itemDataArray[0].textDirection)
    itemDataArray.forEach((itemData, index) => {
      if (itemData.embeddingLevel !== currentLevel) {
        ...
        levelInfo = new LevelInfo(currentLevel, index, -1, itemData.textDirection)
      }
    })
   ...
   * @return {string}
   */
  static detectDefaultTextDirection(boiArray) {
    if (boiArray.length === 0) {
      return ''
    }
    if (boiArray.length === 1) {
   ...
   * @param {BidiOrderInfo[]}boiArray
   */
  static detectDefaultTextDirectionFromLevelInfoArray(levelInfoArray, boiArray) {
    if (boiArray.length === 0) {
      return ''
    }
    if (boiArray.length === 1) {
