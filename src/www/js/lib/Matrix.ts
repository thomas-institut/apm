...
type CellEmptyFunction<T> = (row: number, col: number, value: T) => boolean;
...
export class Matrix<T> {
  ...
  constructor(nRows: number, nCols: number, defaultValue: T) {
    this.nRows = nRows
    this.nCols = nCols
  ...
  isColumnEmpty(col: number, isCellEmptyFunction: CellEmptyFunction<T>) {
    for(let row = 0; row < this.nRows; row++) {
  ...
  isEqualTo(other: T[][]|Matrix<T>): boolean {
    const otherMatrix = other instanceof Matrix ? other : new Matrix(this.nRows, this.nCols, this.defaultValue);
    if (!(other instanceof Matrix)) {
      otherMatrix.setFromArray(other);
    }
