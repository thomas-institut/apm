...
const JsToReact = () => {
  ...
  const images = ['https://loremflickr.com/cache/resized/65535_54226753332_4eaa3fe5e0_b_1000_1000_nofilter.jpg', 'https://loremflickr.com/cache/resized/65535_54151462946_aa622eb51d_h_1000_1000_nofilter.jpg', 'https://loremflickr.com/cache/resized/65535_52753845660_aa219641f1_h_1000_1000_nofilter.jpg', 'https://bilderberg.uni-koeln.de/BOOK-DARE-M-DE-MUC-BSB-Clm.465/1/jpg', 'https://bilderberg.uni-koeln.de/BOOK-DARE-M-DE-MUC-BSB-Clm.465/25/jpg', 'https://bilderberg.uni-koeln.de/BOOK-DARE-M-DE-MUC-BSB-Clm.465/40/DeepZoom'];
  ...
  return (<Container>
    ...
    <div key="toggle-row" style={{display: 'flex', flexDirection: 'row', gap: '10px'}}>
      <div>A row of nice toggles ({numTogglesOn} are on)</div>
      {toggleRefs.current.map((_, i) => (<ReactNiceToggle key={i} onToggle={(status) => handleToggle(i, status)}/>))}
    </div>
    <div key="single-toggle" style={{display: 'flex', flexDirection: 'row', gap: '2em'}}>
      <div>This is a nice toggle</div>
      <ReactNiceToggle ref={niceToggleRef}/>
      <Button size="sm" onClick={() => {
        niceToggleRef.current?.getToggleStatus() ? niceToggleRef.current?.toggleOff() : niceToggleRef.current?.toggleOn();
      }}>Toggle</Button>
    </div>

    <div key="image-chooser" style={{marginTop: '1em'}}>
      <div style={{display: 'flex', flexDirection: 'row', gap: '10px'}}>
        <div>Choose an image</div>
        {images.map((image, i) => (<Button key={i} onClick={() => {
            if (image !== imageUrl) {
    ...
    <div key="viewer" style={{width: '500px', height: '500px', marginTop: '1em', border: '1px solid #dee2e6'}}>
...
const ReactNiceToggle = (props: NiceToggleProps) => {
  ...
  useImperativeHandle(props.ref, () => ({
    toggleOn: () => {
      niceToggleInstanceRef.current?.toggleOn();
    }, toggleOff: () => {
      niceToggleInstanceRef.current?.toggleOff();
    }, getToggleStatus: () => {
      return niceToggleInstanceRef.current?.getToggleStatus() ?? false;
    }
  }), []);
