...
class CollationTest extends TestCase {
    ...
    private function countNonWhiteSpaceTokens(array $tokens): int
    {
        $count = 0;
        foreach ($tokens as $t) {
    ...
    public function doSimpleCollationTests(CollationTable $collation): void
    {
        ...
        $w4 = new SimpleWitness('tw', 'tchunk', []);
        ...
        $exceptionRaised = false;
        try {
            $collation->getWitnessTokens('NonexistentSiglum');
        } catch (InvalidArgumentException) {
            $exceptionRaised = true;
        }
    ...
    public function testCollatexInputGeneration() {
        ...
        $w2 = new StringWitness('tw', 'tchunk', 'This is witness two with more text');
        $w3 = new StringWitness('tw', 'tchunk', 'This witness three');
        $w4  = new SimpleWitness('tw', 'tchunk', [ new Token(3, 'text', 'norm')]);
    ...
    private function getTestCase1JSON(): string
    {
        return <<<EOT
{
    "witnesses": [
